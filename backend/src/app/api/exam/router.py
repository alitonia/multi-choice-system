...
async def start_exam(exam_id: int = Body(..., embed=True), session: AsyncSession = Depends(get_session),
                     principal: Principal = Depends(security.get_current_user)):
    ...
    participant_exam = await participant_exam_service.get_participant_exam(session=session, exam_id=exam_id,
                                                                           examinee_id=account_id)
    if participant_exam:
...
async def answer_question(exam_id: int, body: ExamAnswerSchemaIn, session: AsyncSession = Depends(get_session),
                          principal: Principal = Depends(security.get_current_user)):
    exam_service = Exam_Service(session=session)
    ...
    await choice_service.save_choices(session=session, question_id=question_id, answers_id=answers_id,
                                      examinee_id=account_id)
...
async def finish_exam(body: ExamFinishSchemaIn, session: AsyncSession = Depends(get_session),
                      principal: Principal = Depends(security.get_current_user)):
    exam_service = Exam_Service(session=session)
    question_service = Question_Service(session=session)
    ...
    for question in questions:
        ...
        choices = await choice_service.get_choices(session=session, question_id=question.question_id,
                                                   examinee_id=account_id)
        if not choices:
            continue
    ...
    await participant_exam_service.finish_participant_exam(session=session, participant_exam=participant_exam,
                                                           score=score)
...
async def get_examinee_exam(exam_id: int, session: AsyncSession = Depends(get_session),
                            principal: Principal = Depends(security.get_current_user)):
    exam_service = Exam_Service(session=session)
    question_service = Question_Service(session=session)
