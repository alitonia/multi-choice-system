                ...
                <div className={styles.adminPageUpperLeft}>
                    ...
                    <div>
                        <form onSubmit={submitSearch}>
                            ...
                            />
                            <button type="submit" className={styles['override-but']}>Search</button>
                        </form>
                    </div>
                </div>
                    ...
                    <div>
                        <a href={"/admin/createAccount/"}>
                            <button className={styles['create-new-account-but']}>
                                <i className="fa fa-plus"/><span>Create New Account...</span>
                            </button>
                        </a>
                    </div>
                </div>
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    <a href={"/account/edit/" + item.account_id.toString()}>
                                        <button className={styles['classic-admin-edit-button']}>Edit</button>
                                    </a>
                                </td>
                            </tr>
                        ))}
...
const Pagination = ({currentPage, totalPage, search}) => {
    ...
    if (totalPage < 5)
        return (
            <div>
                {tmp.map((item, index) => (
                    <button className={styles['dashboard-pagination-button']} key={index}
        ...
        if (cP === 1 || cP === 2) {
            return (
                <div>
                    <button className={styles['dashboard-pagination-button']} onClick={() => moveToPage(1, search)}>1
                    </button>
                    <button className={styles['dashboard-pagination-button']} onClick={() => moveToPage(2, search)}>2
                    </button>
                    <button className={styles['dashboard-pagination-button']}
                            onClick={() => moveToPage(3, search)}>...
                    </button>
                    <button className={styles['dashboard-pagination-button']}
                            onClick={() => moveToPage(totalPage, search)}>{totalPage}</button>
                </div>
            );
            ...
            return (
                <div>
                    ...
                    <button className={styles['dashboard-pagination-button']}
                            onClick={() => moveToPage(cP, search)}>{cP}</button>
                    <button className={styles['dashboard-pagination-button']}
                            onClick={() => moveToPage(cP + 1, search)}>...
                    </button>
                    <button className={styles['dashboard-pagination-button']}
