...
const getExamInfo = async examID => {
    ...
    try {
        const res = await axios.get(
            `http://${process.env.REACT_APP_BACKEND_URL}exam/get/${examID}`,
            { headers: headers }
        );
...
const getQuestionInfos = async examID => {
    ...
    try {
        const res = await axios.get(
            `http://${process.env.REACT_APP_BACKEND_URL}questions/get/${examID}`,
            { headers: headers }
        );
    ...
    try {
        if (questionInfo.question_id >= 0) {
            ...
            const res = await axios.put(
                `http://${process.env.REACT_APP_BACKEND_URL}question`,
                updateData,
                { headers: headers }
            );
...
const deleteQuestion = async questionID => {
    try {
        ...
        const res = await axios.delete(`http://${process.env.REACT_APP_BACKEND_URL}question`, {
            headers: headers,
            data: deleteData
        });
