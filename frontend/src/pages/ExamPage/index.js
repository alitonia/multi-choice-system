...
const ExamPage = () => {
    ...
    const startTimeString = (startTime) => {
        ...
        return (trueHour<10?"0":"")+trueHour.toString()+":"+(min<10?"0":"")+min.toString() + " "+dayTime + ", "+(month<10?"0":"")+month.toString() +(day<10?"0":"")+"/"+day.toString()+"/"+year.toString()
    }
    return (
        <div>
            <Header />
            <div className={styles.content}>
                ...
                <button className={styles.beginButton}
                        ...
                        disabled={false}
                >{info.startTime + info.duration*60*1000<Date.now()?"The exam has been expired":(
                    info.startTime>Date.now()?"Not the time yet":"Begin the exam"
                )
                }</button>
            </div>
